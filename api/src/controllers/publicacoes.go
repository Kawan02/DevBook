...
func AtualizarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	repositorio := repositories.NovoRepositorioDePublicacoes(db)
	publicacaoSalvoNoBanco, erro := repositorio.BuscarPorID(publicacaoID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}

	if publicacaoSalvoNoBanco.AutorID != usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New(
			"não é possível atualizar uma publicação que não seja sua"),
		)
		return
	}
...
func DeletarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	if publicacaoSalvoNoBanco.AutorID != usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New(
			"não é possível deletar uma publicação que não seja sua"),
		)
		return
	}
...
// BuscarPublicacaoPorUsuario traz todas as publicações de um usuário especifíco
